...
class AcceloTasks extends Model
{
    ...
    public static function getAcceloDBTasks($type='', $status=0){
      #developer demo
      /*$project_id = 43;
      ...
      return $records;*/
      if($type){
        $records = AcceloTasks::where('status', $status)->where('type', $type)->limit(config('accelohub.cLimit'))->get()->map(function($task){
          $project = AcceloProjects::where('id', $task->project_id)->first();
          $task['accelo_project_id']    = $project->accelo_project_id;
          $task['hubstaff_project_id']  = $project->hubstaff_project_id;
          return $task;
        ...
        $records = AcceloTasks::where('status', $status)->limit(config('accelohub.cLimit'))->get()->map(function($task){
          $project = AcceloProjects::where('id', $task->project_id)->first();
          $task['accelo_project_id']    = $project->accelo_project_id;
          $task['hubstaff_project_id']  = $project->hubstaff_project_id;
          return $task;
